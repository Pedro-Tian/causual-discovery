...
from castle.algorithms.ges.functional.graph import pdag_to_dag
...
def merge_graph_voting(sub_nodes_list, sub_causal_matrix_list, true_dag):
    ...
    for nodes, sub_causal_matrix in zip(sub_nodes_list, sub_causal_matrix_list):
        ...
        if nodes is None or sub_causal_matrix is None: continue
...
def infer_causal(args, X, true_dag):
    ...
    if args.model == 'CORL':
        ...
        model = GES()
        model.learn(X)
        causal_matrix = pdag_to_dag(model.causal_matrix)
        causal_matrix_order = model.causal_matrix
        # help(model)
        # exit()
        # print(causal_matrix)
        ...
        # print(causal_matrix)
        logger.info(f"check dag: {check_dag(causal_matrix)}")
        # if not check_dag(causal_matrix):
        #     # help(GraphDAG)
        #     GraphDAG(causal_matrix, true_dag, save_name='test.png')
